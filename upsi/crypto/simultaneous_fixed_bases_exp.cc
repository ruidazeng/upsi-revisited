...
#include "upsi/crypto/simultaneous_fixed_bases_exp.h"

#include <algorithm>
#include <memory>
#include <vector>

#include "upsi/crypto/mont_mul.h"
...
template <>
StatusOr<upsi::BigNum> Clone(
    const upsi::BigNum& element) {
  return element;
}

template <>
bool IsZero(const upsi::BigNum& c) {
  return c.IsOne();
}
...
template <>
StatusOr<upsi::MontBigNum> Clone(
    const upsi::MontBigNum& element) {
  return element;
}

template <>
StatusOr<upsi::MontBigNum> Mul(
    const upsi::MontBigNum& e1,
    const upsi::MontBigNum& e2,
    const upsi::MontContext& context) {
  return e1.Mul(e2);
}

template <>
bool IsZero(const upsi::MontBigNum& c) {
...
StatusOr<Element> SimultaneousFixedBasesExp<Element, Context>::SimultaneousExp(
    const std::vector<upsi::BigNum>& exponents) const {
...
template class SimultaneousFixedBasesExp<upsi::MontBigNum,
                                         upsi::MontContext>;
