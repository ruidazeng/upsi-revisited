 ...
 protected:
  void SetUp() override {
    ASSERT_OK_AND_ASSIGN(
        auto ec_group,
        upsi::ECGroup::Create(kTestCurveId, &ctx_));
    ec_group_ = std::make_unique<upsi::ECGroup>(
        std::move(ec_group));
  }

  upsi::Context ctx_;
  std::unique_ptr<upsi::ECGroup> ec_group_;
};

TEST_F(SimultaneousFixedBasesExpTest, ZnMultipleExp) {
  upsi::BigNum n = ctx_.CreateBigNum(P);  // Prime modulus.
  auto base1 = ctx_.GenerateRandLessThan(n);
  auto base2 = ctx_.GenerateRandLessThan(n);
  upsi::BigNum exponent1 = ctx_.CreateBigNum(29);
  upsi::BigNum exponent2 = ctx_.CreateBigNum(2245);
  ...
  std::vector<upsi::BigNum> exponents;
