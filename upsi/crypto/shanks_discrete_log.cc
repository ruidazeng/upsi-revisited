...
#include "upsi/crypto/shanks_discrete_log.h"
...
ShanksDiscreteLog::ShanksDiscreteLog(
    upsi::Context* ctx,
    const upsi::ECGroup* group,
    std::unique_ptr<upsi::ECPoint> generator,
...
absl::StatusOr<std::map<std::string, int>> ShanksDiscreteLog::PrecomputeTable(
    const upsi::ECGroup* group,
    const upsi::ECPoint* generator, int precompute_bits) {
...
absl::StatusOr<std::unique_ptr<ShanksDiscreteLog>> ShanksDiscreteLog::Create(
    upsi::Context* ctx,
    const upsi::ECGroup* group,
    const upsi::ECPoint* generator, int max_message_bits,
    int precompute_bits) {
  ...
  auto generator_ptr = std::make_unique<upsi::ECPoint>(
