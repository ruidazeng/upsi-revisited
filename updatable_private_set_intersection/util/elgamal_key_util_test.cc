...
using updatable_private_set_intersection::ElGamalPublicKey;
using updatable_private_set_intersection::ElGamalSecretKey;
...
TEST(ElGamalKeyUtilTest, TestKeyReadWrite) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto key_pair, updatable_private_set_intersection::elgamal::GenerateKeyPair(group));
  ASSERT_OK_AND_ASSIGN(
      auto public_key_proto,
