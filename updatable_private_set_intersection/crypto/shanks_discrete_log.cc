...
#include "updatable_private_set_intersection/crypto/shanks_discrete_log.h"
...
ShanksDiscreteLog::ShanksDiscreteLog(
    updatable_private_set_intersection::Context* ctx,
    const updatable_private_set_intersection::ECGroup* group,
    std::unique_ptr<updatable_private_set_intersection::ECPoint> generator,
...
absl::StatusOr<std::map<std::string, int>> ShanksDiscreteLog::PrecomputeTable(
    const updatable_private_set_intersection::ECGroup* group,
    const updatable_private_set_intersection::ECPoint* generator, int precompute_bits) {
...
absl::StatusOr<std::unique_ptr<ShanksDiscreteLog>> ShanksDiscreteLog::Create(
    updatable_private_set_intersection::Context* ctx,
    const updatable_private_set_intersection::ECGroup* group,
    const updatable_private_set_intersection::ECPoint* generator, int max_message_bits,
    int precompute_bits) {
  ...
  auto generator_ptr = std::make_unique<updatable_private_set_intersection::ECPoint>(
...
absl::StatusOr<int64_t> ShanksDiscreteLog::GetDiscreteLog(
    const updatable_private_set_intersection::ECPoint& point) {
